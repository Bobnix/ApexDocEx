...
public class MethodModelTest {
    ...
    public void testSplitExceptions(){
        List<String> testExceptions = new ArrayList<String>();
        testExceptions.add("Exception withSpace");
        testExceptions.add("Exception      withMultipleSpace");
        testExceptions.add("Exception\t withTab");

        MethodModel model = new MethodModel();
        model.setExceptions(testExceptions);

        List<List<String>> retVal = model.getSplitExceptions();

        Assert.assertEquals(retVal.get(0).get(0), "Exception");
        Assert.assertEquals(retVal.get(0).get(1), "withSpace");
        Assert.assertEquals(retVal.get(1).get(0), "Exception");
        Assert.assertEquals(retVal.get(1).get(1), "withMultipleSpace");
        Assert.assertEquals(retVal.get(2).get(0), "Exception");
        Assert.assertEquals(retVal.get(2).get(1), "withTab");
    }
	...
	public void testMergeDocBlockData(){
        ...
        testData.put("@throws", Arrays.asList("Exception 1", "Exception 2"));
        ...
        Assert.assertTrue(model.getExceptions().contains("Exception 1"));
        Assert.assertTrue(model.getExceptions().contains("Exception 2"));
